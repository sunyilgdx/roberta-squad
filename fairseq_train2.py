...
class RobertaQAEmbed(FairseqDecoder):
    ...
    def forward(self, q=None, a=None, return_loss=False, **kwargs):
        ...
        if has_q:
          q_hs = torch.stack(self.sentence_encoder(q, last_state_only=False)[0][-4:]).mean(0)[:,0,:]  # [bs, hs]
        ...
        if has_a:
          a_hs = torch.stack(self.sentence_encoder(a, last_state_only=False)[0][-4:]).mean(0)[:,0,:]  # [bs, hs]
